...
export default class Purchase extends Component {
    ...
    goTrade = () => {
        const {navigation} = this.props;
        // fetch("http://10.0.2.2:8000/charge", {
            fetch('http://119.23.231.235/api/charge',{
            method: 'POST',
            headers: {
                'Accept': 'application/json',
        ...
        return (
            <View style={styles.container}>
                <KeyboardAwareScrollView >
                    ...
                    <Text style={{position:'absolute', top:150, fontSize:30, fontWeight:'600', left:15, color:'#fff'}}>CHARGE MONEY 资金充值</Text>
                    <List renderHeader={() => '充值基本信息'}>
                        <InputItem
                            clear
                    ...
                    <List renderHeader={() => '完善信息进行充值'}>
                        <List.Item>
                            <Text
                                style={{paddingBottom: 10}}
                            >充值金额:{this.state.cost}</Text>
                            <Slider
